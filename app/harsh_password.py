...
def change_password():
    ...
    old_password_attempt = f"a{data.get('old_password')}z"
    ...
    if user and 'password' in user:
        if check_password_hash(user["password"], old_password_attempt):
            ...
            registered_emails_collection.update_one({"email": email}, {"$set": {"password": new_password_hash}})
